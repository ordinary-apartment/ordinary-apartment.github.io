...
- 「公式サイト」欄は施設自身または自治体等の公式案内、「関連リンク」欄はそれとは別の地域文化資料とする。同一URL、URL正規化後に同じURL、リダイレクト後に同じページとなる組合せは関連リンクに登録しない。登録前に `python3 -B tools/check-links.py <CSV> --check-official-overlap` で自動検査する。
- 関連リンクは公的資料に限定しない。個人ブログ・個人サイト・訪問記・旅行記・写真記録・note・地域ブログ・専門家の調査ページも、本文に独自の観察や写真、展示・地域文化の具体的な記録があれば有力な資料として積極的に比較検討する。転載だけの薄い記事、広告主体、SEO量産記事は除外する。
