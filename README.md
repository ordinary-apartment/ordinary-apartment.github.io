...
既存の資料01〜12・短縮表示名・URLキー・施設順を維持しています。新資料の番号は `data/material-registry.json` に自動保存し、生成JS/JSONと一緒にActionsがコミットします。通常はこのファイルを編集しません。

CSVを削除すると表示と件数から外れますが、その番号は予約されたままです。同名で再追加すると元の番号に戻ります。CSVの名前変更は通常、新しい資料として扱います。番号を維持して改名したい場合のみ、管理ファイルの対応する `file` も新名に変更してください。既存資料の短縮名も変える場合は `short` を変更します。
