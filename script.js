...
const relatedLinksMarkup=(links,compact=false)=>links?.length?`<span class="related-links">${links.slice(0,3).map(link=>{const label=compact?'■':h(link.title);return `<a class="related-link${compact?' related-link-mark':''}" href="${h(link.url)}" target="_blank" rel="noopener noreferrer" title="${h(link.title)}" aria-label="${h(link.title)}">${label}</a>`}).join('')}</span>`:'—';
