...
const relatedLinksMarkup=(links,compact=false)=>links?.length?`<span class="related-links">${links.slice(0,3).map(link=>{const wiki=/https?:\/\/(?:ja|en)\.wikipedia\.org\/wiki\//i.test(link.url);const label=wiki?'wiki':(compact?'■':h(link.title));return `<a class="related-link${compact?' related-link-mark':''}" href="${h(link.url)}" target="_blank" rel="noopener noreferrer" title="${h(link.title)}" aria-label="${h(link.title)}">${label}</a>`}).join('')}</span>`:'—';
const wikipediaLinksMarkup=extra=>{
  const text=extra?.['参照']||'';
  const urls=[...text.matchAll(/https?:\/\/(?:ja|en)\.wikipedia\.org\/wiki\/[^\s\n]+/gi)].map(m=>m[0]);
  return urls.length?`<span class="reference-links">${urls.map(url=>{let title=url.split('/wiki/')[1]||'';try{title=decodeURIComponent(title).replace(/_/g,' ')}catch{};return `<a class="reference-link" href="${h(url)}" target="_blank" rel="noopener noreferrer" title="${h(title+' - Wikipedia')}" aria-label="${h(title+' - Wikipedia')}">wiki</a>`}).join('')}</span>`:'';
};
const referenceMarkup=(extra,compact=false)=>wikipediaLinksMarkup(extra)||'';
function catalogSection(number,id,title,note,items){const rows=items.map(x=>`<tr data-prefecture="${h(x.prefecture)}" data-type="${h(x.type)}"><td>${x.official?`<a href="${h(x.official)}" target="_blank" rel="noopener noreferrer">公式</a> `:''}<a href="${h(x.maps)}" target="_blank" rel="noopener noreferrer">地図</a> ${referenceMarkup(x.extra)}</td><td>${h(x.prefecture)}</td><td>${h(x.city)}</td><td>${x.name==='玄海海中展望塔'?`<button class="facility-detail-link" data-facility-detail="genkai-undersea">${h(x.name)}</button>`:h(x.name)}</td><td>${h(x.type)}</td><td>${relatedLinksMarkup(x.relatedLinks,true)}</td><td>${h(x.note)}</td></tr>`).join('');return `<section class="reference-section" id="${id}"><div class="reference-heading"><span>資料${number}</span><h2>${title}</h2><span>${String(items.length).padStart(3,'0')}件</span></div><p class="reference-note">${note}　都道府県・分類の見出しで昇順／降順。</p><div class="record-table catalog-table"><table>${sortableHead}<tbody>${rows}</tbody></table></div></section>`}
