...
const relatedLinksMarkup=(links,compact=false)=>links?.length?`<span class="related-links">${links.slice(0,3).map(link=>`<a class="related-link${compact?' related-link-mark':''}" href="${h(link.url)}" target="_blank" rel="noopener noreferrer" title="${h(link.title)}" aria-label="${h(link.title)}">${compact?'■':h(link.title)}</a>`).join('')}</span>`:'—';
const wikipediaLinksMarkup=extra=>{
  const text=extra?.['参照']||'';
  const urls=[...text.matchAll(/https?:\/\/(?:ja|en)\.wikipedia\.org\/wiki\/[^\s\n]+/gi)].map(m=>m[0]);
  return urls.length?`<span class="reference-links">${urls.map(url=>{let title=url.split('/wiki/')[1]||'';try{title=decodeURIComponent(title).replace(/_/g,' ')}catch{};return `<a class="reference-link" href="${h(url)}" target="_blank" rel="noopener noreferrer" title="${h(title+' - Wikipedia')}" aria-label="${h(title+' - Wikipedia')}">${h(title+' - Wikipedia')}</a>`}).join('')}</span>`:'';
};
